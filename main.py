...
<script>
  ...
  function InitThis() {
      ...
      emocion = ["Feliz", "Triste", "Enojado", "Miedo", "Desagrado"];
      random = Math.floor(Math.random() * emocion.length);
      aleatorio = emocion[random];

      document.getElementById('mensaje').innerHTML  = 'Dibuja la siguiente emoción: ' + aleatorio;
...
def prepare_dataset():
    images = []
    emociones = ["Feliz", "Triste", "Enojado", "Miedo", "Desagrado"]
    digits = []
    for emocion in emociones:
      filelist = glob.glob('{}/*.png'.format(emocion))
      images_read = io.concatenate_images(io.imread_collection(filelist))
      images_read = images_read[:, :, :, 3]
      digits_read = np.array([emocion] * images_read.shape[0])
